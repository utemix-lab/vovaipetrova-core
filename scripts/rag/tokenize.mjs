...
function normalizeText(text) {
  ...
  // 8. Нормализация ё/е (v2: более тонкая обработка)
  // Заменяем ё на е только в контексте, где это не меняет смысл
  // Для RAG сохраняем оба варианта в индексе, но нормализуем запросы
  normalized = normalized.replace(/ё/g, 'е');
  normalized = normalized.replace(/Ё/g, 'Е');

  // 9. Нормализация дефисов в составных словах (v2)
  // Сохраняем дефисы в составных словах, но нормализуем тире
  normalized = normalized.replace(/([а-яА-Яa-zA-Z])-([а-яА-Яa-zA-Z])/g, '$1-$2'); // Сохраняем дефисы в словах

  // 10. Нормализация латиницы/кириллицы (v2: не смешиваем)
  // Оставляем как есть, но нормализуем пробелы вокруг

  // 11. Удаление лишних пробелов вокруг пунктуации
  normalized = normalized.replace(/\s+([.,!?;:])/g, '$1');
  normalized = normalized.replace(/([.,!?;:])\s{2,}/g, '$1 ');

  // 12. Финальная нормализация пробелов (сохраняем переносы строк)
...
/**
 * Экспорт normalizeText для использования в других модулях
 */
...
function runTests() {
  const tests = [
    ...
    {
      name: 'Нормализация ё/е',
      input: 'Текст с буквой ё и Ё',
      expected: 'Текст с буквой е и Е',
    },
    {
      name: 'Дефисы в составных словах',
      input: 'Русско-английский текст',
      expected: 'Русско-английский текст',
    },
    {
      name: 'Пустая строка',
      input: '',
