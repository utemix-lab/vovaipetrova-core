#!/usr/bin/env node
/**
 * CI Re-run Helper: перезапуск PR с очисткой старых CI комментариев
 *
 * Удаляет старые комментарии от CI ботов (github-actions[bot]),
 * оставляет комментарии Cursor Bugbot, и перезапускает CI проверки.
 *
 * Использование:
 *   node scripts/pr-rerun.mjs [--pr=<number>] [--dry-run] [--skip-clear]
 *
 * Примеры:
 *   node scripts/pr-rerun.mjs --pr=145
 *   node scripts/pr-rerun.mjs --pr=145 --dry-run
 *   node scripts/pr-rerun.mjs --pr=145 --skip-clear  # только перезапуск CI
 *
 * Или через npm:
 *   npm run pr:rerun -- --pr=145
 */
...
function rerunCI(prNumber, dryRun) {
  ...
  try {
    ...
    const repo = prData.headRepository.nameWithOwner || GITHUB_REPO;
...
async function main() {
  ...
  const { pr, dryRun, skipClear } = args;

  if (!pr) {
    log('❌ Укажите номер PR: --pr=<number>');
    log('');
    log('Примеры:');
    log('  node scripts/pr-rerun.mjs --pr=145');
    log('  node scripts/pr-rerun.mjs --pr=145 --dry-run');
    log('  node scripts/pr-rerun.mjs --pr=145 --skip-clear');
    log('');
    log('Или через npm:');
    log('  npm run pr:rerun -- --pr=145');
    process.exit(1);
  }
  ...
  log(`🔄 Перезапуск PR #${pr} с очисткой старых CI комментариев`);
  log('');
