#!/usr/bin/env node
/**
 * Sandbox test для проверки CodeGPT guardrails
 * Тестирует size-guard для CodeGPT задач
 * 
 * Использование:
 *   node scripts/test-codegpt-guardrails-sandbox.mjs
 */
...
const SANDBOX_DIR = 'tests/sandbox/codegpt-guardrails';
...
function createTestBranch() {
  try {
    ...
    const branches = execSync('git branch --list test-codegpt-guardrails', { encoding: 'utf-8' }).trim();
    if (branches) {
      execSync('git checkout test-codegpt-guardrails', { stdio: 'pipe' });
      execSync('git reset --hard main', { stdio: 'pipe' });
    } else {
      execSync('git checkout -b test-codegpt-guardrails', { stdio: 'pipe' });
    }
    
    // Создаём тестовые файлы для проверки size-guard
    const testFiles = [
      { path: '.codegpt/test-agent.md', content: '# Test Agent\n\nTest content.\n'.repeat(50) },
      { path: 'docs/codegpt-test.md', content: '# CodeGPT Test\n\nTest content.\n'.repeat(100) },
      { path: 'scripts/codegpt/test-script.mjs', content: '// Test script\n'.repeat(30) },
    ];
    ...
    execSync('git commit -m "test: CodeGPT guardrails sandbox test"', { stdio: 'pipe' });
...
function cleanupTestBranch() {
  try {
    ...
    execSync('git branch -D test-codegpt-guardrails', { stdio: 'pipe' });
  } catch (error) {
...
function getTestDiffStats() {
  try {
    const command = 'git diff --numstat main...test-codegpt-guardrails';
...
function main() {
  console.log('🧪 CodeGPT Guardrails Sandbox Test\n');
  ...
  try {
    ...
    // Проверяем size-guard для CodeGPT
    console.log('🔍 Testing size-guard for CodeGPT...');
    const sizeCheck = checkSizeGuard(stats, 'codegpt');
    ...
    const reportPath = join(RESULTS_DIR, 'codegpt-guardrails-sandbox-report.md');
    let reportMarkdown = `# CodeGPT Guardrails Sandbox Test Report\n\n`;
    reportMarkdown += `Generated: ${report.timestamp}\n\n`;
    reportMarkdown += `## Test Results\n\n`;
    reportMarkdown += `### Size Guard (CodeGPT)\n\n`;
    reportMarkdown += `**Limits:** ${sizeCheck.limits.maxFiles} files, ${sizeCheck.limits.maxAdditions} additions, ${sizeCheck.limits.maxDeletions} deletions\n\n`;
    ...
    if (totalViolations === 0) {
      reportMarkdown += `✅ **All guardrails passed!** CodeGPT guardrails are working correctly.\n`;
    } else {
    ...
    if (totalViolations === 0) {
      console.log('\n✅ Sandbox test passed: CodeGPT guardrails are working correctly');
      process.exit(0);
    } else {
