...
### ⚠️ ОБЯЗАТЕЛЬНО ПЕРЕД ПРЕДСТАВЛЕНИЕМ PR НА ПРОВЕРКУ (особенно после правок)
- [ ] **Удалены старые комментарии ботов**: После любых правок в PR **ОБЯЗАТЕЛЬНО** удалить все старые комментарии от ботов (GitHub Actions, Cursor Bot) перед представлением PR на проверку. Это правило проекта: PR должен быть представлен только с актуальными комментариями, без старых исправленных ошибок.

**Как удалить старые комментарии ботов:**
```bash
# Вариант 1: Через GitHub CLI (рекомендуется)
# Получить номер PR (из URL или ветки)
PR_NUMBER=<номер-PR>

# Получить список комментариев
gh pr view $PR_NUMBER --comments --json comments > tmp-comments.json

# Удалить комментарии от ботов (требуется скрипт для фильтрации)
# Фильтровать по авторам: "github-actions[bot]", "cursor-bot", "cursor[bot]"
# Для каждого комментария от бота выполнить:
gh api repos/:owner/:repo/issues/$PR_NUMBER/comments/<comment-id> -X DELETE

# Вариант 2: Вручную через веб-интерфейс GitHub
# Открыть PR → найти комментарии от ботов → удалить каждый вручную
```

**После удаления комментариев перезапустить CI:**
```bash
# Создать пустой коммит для перезапуска CI
git commit --allow-empty -m "chore: trigger CI recheck after clearing bot comments"
git push
```

**Важно**: Это правило проекта для предотвращения путаницы между исправленными и актуальными проблемами. После правок всегда перезапускать PR с удалением старых комментариев.
