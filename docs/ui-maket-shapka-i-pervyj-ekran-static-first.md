...
summary: >-
  Статический прототип первого экрана с переключением ролей, корзиной артефактов
  и набором UI-компонентов без серверной части.
tags:
  - Figma
  - UX
machine_tags:
...
## TL;DR
- Прототип шапки и первого экрана со переключателем ролей и корзиной артефактов.
- Чистый статический стек: HTML/CSS/JS + JSON с данными.
- Сценарии ролей (Новичок, Клиент, Dev) подгружаются из `data/roles.json`.
- Используется для быстрых демонстраций и smoke-тестов перед интеграцией.
