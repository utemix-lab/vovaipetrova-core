...
- **Notion Briefs**: карточка в статусе `Ready` с полями `Brief`, `Scope`, `Deliverables`, `Executor`, `Lane`, `SLA`, `Requires Review`, `RFC Link`, `Upstream Source`
  ...
  - **ОБЯЗАТЕЛЬНО**: Если не удаётся получить задачу из Notion через MCP (сбой связи, MCP, Wi-Fi и т.д.) — уведомить автора (пользователя) о проблеме с связью. Сценарий должен быть один — всегда из Notion Briefs. Если связь не работает, автор починит её.
...
- **Чтение комментариев ботов**: Исполнители (Copilot, Cursor и другие) могут самостоятельно читать комментарии проверяющих ботов (CI и Cursor Bugbot) через `gh pr view <номер> --comments` и GitHub API. После создания PR и после правок исполнитель должен самостоятельно читать все комментарии ботов и исправлять ошибки. **Важно**: Комментарии ботов появляются не сразу — нужно время на проверку. Исполнители не привязаны к текущему времени и не могут проверить комментарии "спустя время" автоматически. Поэтому пользователь может просто сказать "проверь комментарии в PR #<номер>" или "исправь ошибки", и исполнитель прочитает комментарии и исправит ошибки. Cursor Bugbot является сторонней проверкой, его комментарии не удалить — ориентироваться по времени комментариев (новые комментарии после последних правок).
...
- ❌ **Не игнорировать комментарии ботов после правок** — после внесения правок и перезапуска PR исполнитель должен самостоятельно прочитать все комментарии ботов через `gh pr view <номер> --comments` и исправить ошибки. Если пользователь говорит "проверь комментарии" или "исправь ошибки", исполнитель читает комментарии и исправляет их. Исполнители могут различать старые и новые комментарии по времени (новые комментарии после последних правок).
...
1. ✅ **Перезапустить PR**: Создать пустой коммит `git commit --allow-empty -m "chore: trigger CI recheck"` и запушить `git push`. Это возобновляет проверку CI-ботов.
2. ✅ **Прочитать комментарии ботов**: После перезапуска (когда комментарии появятся, обычно через 2-5 минут) исполнитель должен самостоятельно прочитать все комментарии через `gh pr view <номер> --comments` и исправить ошибки. Если пользователь говорит "проверь комментарии" или "исправь ошибки", исполнитель читает комментарии и исправляет их.
...
**Проблемы с связью Notion:**
- Если не удаётся получить задачу из Notion через MCP (сбой связи, MCP, Wi-Fi и т.д.) — уведомить автора (пользователя) о проблеме с связью
- Сценарий должен быть один — всегда из Notion Briefs
- Если связь не работает, автор починит её
- Не начинать работу без задачи из Notion Briefs
...
1. **Чтение задачи**: получить Brief из Notion через MCP (`notion_search` или `notion_fetch`)
   - **ОБЯЗАТЕЛЬНО**: Задачу брать из Notion Briefs через MCP
   - **ОБЯЗАТЕЛЬНО**: Перед началом работы проверить текущую нумерацию PR через `gh pr list` для корректного именования веток
   - **ОБЯЗАТЕЛЬНО**: Если не удаётся получить задачу из Notion (сбой связи, MCP, Wi-Fi и т.д.) — уведомить автора (пользователя) о проблеме с связью и не начинать работу. Сценарий должен быть один — всегда из Notion Briefs. Если связь не работает, автор починит её.
...
10. **Ожидание проверки ботами**: 
    - **ОБЯЗАТЕЛЬНО**: PR проверяется ботами CI и Cursor Bugbot. После создания PR и после правок исполнитель должен самостоятельно читать все комментарии ботов через `gh pr view <номер> --comments` и исправлять ошибки. **Важно**: Комментарии ботов появляются не сразу — нужно время на проверку (обычно 2-5 минут). Исполнители не привязаны к текущему времени и не могут проверить комментарии "спустя время" автоматически. Поэтому пользователь может просто сказать "проверь комментарии в PR #<номер>" или "исправь ошибки", и исполнитель прочитает комментарии и исправит ошибки. Cursor Bugbot является сторонней проверкой, его комментарии не удалить — ориентироваться по времени комментариев (новые комментарии после последних правок).
    - После правок нужно перезапускать PR (именно перезапускать, а не делать новый) через пустой коммит `git commit --allow-empty -m "chore: trigger CI recheck"` и `git push`. Это возобновляет проверку CI-ботов.
