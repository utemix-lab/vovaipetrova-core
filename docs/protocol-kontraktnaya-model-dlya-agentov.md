...
- **Чтение комментариев ботов**: Исполнители (Copilot, Cursor и другие) могут самостоятельно читать комментарии проверяющих ботов (CI и Cursor Bugbot) через `gh pr view <номер> --comments` и GitHub API. После создания PR и после правок исполнитель должен самостоятельно читать все комментарии ботов и исправлять ошибки. **Важно**: Комментарии ботов появляются не сразу — нужно время на проверку (обычно 2-5 минут). Когда пользователь говорит "проверь комментарии в PR #<номер>" или "исправь ошибки", исполнитель читает комментарии и исправляет их. Cursor Bugbot является сторонней проверкой, его комментарии не удалить — ориентироваться по времени комментариев (новые комментарии после последних правок).
