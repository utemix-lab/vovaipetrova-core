...
**Важно**: 
- Описание PR должно быть на **русском языке**. Это правило проекта для единообразия и удобства команды. Заголовок PR может быть на английском (для совместимости с инструментами), но описание (body) — обязательно на русском.
- **После правок и перезапуска PR ОБЯЗАТЕЛЬНО удалить старые комментарии ботов**: Выполнить команду `npm run pr:clear-bot-comments -- --pr=<номер>` для удаления всех старых комментариев от ботов (GitHub Actions, Cursor Bot). Комментарии становятся "старыми" после внесения правок и перезапуска проверок — их нужно удалить, чтобы PR содержал только актуальные комментарии без уже исправленных ошибок. Это правило проекта для предотвращения путаницы между исправленными и актуальными проблемами.
