...
- **Перед представлением PR на проверку ОБЯЗАТЕЛЬНО удалить старые комментарии ботов**: Выполнить команду `npm run pr:clear-bot-comments -- --pr=<номер>` для удаления всех старых комментариев от ботов (GitHub Actions, Cursor Bot). PR должен быть представлен на проверку только с актуальными комментариями, без старых исправленных ошибок. Это правило проекта для предотвращения путаницы между исправленными и актуальными проблемами.
...
- ❌ **Не представлять PR на проверку без удаления старых комментариев ботов** — обязательно выполнить `npm run pr:clear-bot-comments -- --pr=<номер>` перед представлением PR на проверку. Это правило проекта для предотвращения путаницы между исправленными и актуальными проблемами.
...
### Перед представлением PR на проверку (ОБЯЗАТЕЛЬНО)
1. ✅ **Удалить старые комментарии ботов**: Выполнить `npm run pr:clear-bot-comments -- --pr=<номер>` для удаления всех старых комментариев от ботов (GitHub Actions, Cursor Bot). Это правило проекта: PR должен быть представлен на проверку только с актуальными комментариями, без старых исправленных ошибок. Это предотвращает путаницу между исправленными и актуальными проблемами.
