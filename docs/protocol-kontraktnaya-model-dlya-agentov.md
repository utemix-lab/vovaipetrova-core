...
- ❌ **Не представлять PR на проверку без удаления старых комментариев ботов** — после любых правок в PR ОБЯЗАТЕЛЬНО удалить все старые комментарии от ботов перед представлением PR на проверку
...
### После правок в PR (ОБЯЗАТЕЛЬНО перед представлением на проверку)

**Правило проекта**: После любых правок в PR **ОБЯЗАТЕЛЬНО** удалить все старые комментарии от ботов и перезапустить CI. PR должен быть представлен на проверку только с актуальными комментариями, без старых исправленных ошибок.

**Шаг 1: Удалить старые комментарии ботов**

Используйте GitHub CLI или веб-интерфейс:

```bash
# Определить номер PR (из URL или ветки)
PR_NUMBER=<номер-PR>

# Получить список комментариев PR
gh pr view $PR_NUMBER --comments --json comments > tmp-comments.json

# Удалить комментарии от ботов
# Авторы ботов для фильтрации:
# - "github-actions[bot]"
# - "cursor-bot"
# - "cursor[bot]"
# - Любые аккаунты, содержащие "[bot]" или "-bot" в имени

# Для каждого комментария от бота выполнить:
gh api repos/:owner/:repo/issues/$PR_NUMBER/comments/<comment-id> -X DELETE
```

**Альтернатива**: Удалить комментарии вручную через веб-интерфейс GitHub (открыть PR → найти комментарии от ботов → удалить каждый).

**Шаг 2: Перезапустить CI проверки**
...
**Альтернатива**: Перезапустить workflow через GitHub Actions веб-интерфейс (Actions → найти последний run → Rerun).
...
4. ✅ **После правок**: старые комментарии ботов удалены, CI перезапущен (см. раздел "После правок в PR")
...
8. **Синхронизация статуса**: обновить статус в Notion на `Review` (если задача из Notion, Stream 2)
   - Copilot: использовать MCP инструмент `notion_update_page`
9. **После правок в PR (ОБЯЗАТЕЛЬНО перед представлением на проверку)**: удалить старые комментарии ботов и перезапустить CI
   - Удалить все комментарии от ботов (GitHub Actions, Cursor Bot) через GitHub CLI или веб-интерфейс
   - Перезапустить CI через пустой коммит: `git commit --allow-empty -m "chore: trigger CI recheck after clearing bot comments" && git push`
   - **Важно**: Это правило проекта — PR должен быть представлен только с актуальными комментариями
