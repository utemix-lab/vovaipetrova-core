...
**Важное правило проекта**: После любых правок в PR **ОБЯЗАТЕЛЬНО** удалить все старые комментарии от ботов и перезапустить CI перед представлением PR на проверку. PR должен быть представлен только с актуальными комментариями, без старых исправленных ошибок.

### Как удалить старые комментарии ботов

**Через GitHub CLI:**
```bash
# 1. Определить номер PR
PR_NUMBER=<номер-PR>

# 2. Получить список комментариев
gh pr view $PR_NUMBER --comments --json comments > tmp-comments.json

# 3. Удалить комментарии от ботов
# Фильтровать по авторам: "github-actions[bot]", "cursor-bot", "cursor[bot]"
# Для каждого комментария от бота:
gh api repos/:owner/:repo/issues/$PR_NUMBER/comments/<comment-id> -X DELETE
```

**Вручную через веб-интерфейс:**
- Открыть PR в GitHub
- Найти все комментарии от ботов (GitHub Actions, Cursor Bot)
- Удалить каждый комментарий вручную
...
- Открыть Actions → найти последний workflow run для PR
- Нажать "Rerun" для перезапуска

### Почему это важно

Старые комментарии от ботов могут относиться к уже исправленным ошибкам. Если они остаются в PR, возникает путаница: какие проблемы актуальны, а какие уже исправлены. Удаление старых комментариев и перезапуск CI гарантирует, что в PR отображаются только актуальные проблемы.
